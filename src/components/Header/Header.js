...
const getUserWait = createSelector(
  [getRoundRobinQueue, getSongs, getPosition, getQueueId, getUserId, getWaits],
  (queue, queueId, userId, waits, songs,pos) => {
    console.log(queue.entities)
    /*let duration = 0
    if (queue.entities.length > 0 && queue.entities[queueId].youtubeVideoId) {
